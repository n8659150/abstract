export default {

        'sliderData':[
            {'sliderSrc':'static/images/thumbs/featured/featured-1.jpg','sliderDate':'September 06, 2016','sliderAuthor':'Naruto Uzumaki','type':'single-standard','sliderTitle':'Minimalism Never Goes Out of Style'},
            {'sliderSrc':'static/images/thumbs/featured/featured-2.jpg','sliderDate':'August 29, 2016','sliderAuthor':'Sasuke Uchiha','type':'single-standard','sliderTitle':'Enhancing Your Designs with Negative Space'},
            {'sliderSrc':'static/images/thumbs/featured/featured-3.jpg','sliderDate':'August 27, 2016','sliderAuthor':'Naruto Uzumaki','type':'single-standard','sliderTitle':'Music Album Cover Designs for Inspiration'}
        ],
        'Liberty':{
            'type': 'single-standard',
            'srcAlt':'Liberty',
            'thumbnailSrc':'static/images/thumbs/liberty.jpg',
            'catLink1':'Branding',
            'catLink2':'html',
            'entryTitle':'Designing With Black and White.',
            'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
                              ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
                              in anim.`
        },
        'Lighthouse':{
            'type': 'single-standard',
            'srcAlt':'Lighthouse',
            'thumbnailSrc':'static/images/thumbs/lighthouse.jpg',
            'catLink1':'Photography',
            'catLink2':'Design',
            'entryTitle':'Breathtaking Photos of Lighthouses.',
            'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
                              ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
                              in anim.`
        },
        'bokeh':{
            'type': 'single-video',
            'srcAlt':'bokeh',
            'thumbnailSrc':'static/images/thumbs/ottawa-bokeh.jpg',
            'catLink1':'Design',
            'catLink2':'Branding',
            'entryTitle':'This Is a Video Post Format.',
            'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
                              ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
                              in anim.`
        },
        'Pattern':{
            'type': 'single-standard',
            'srcAlt':'Pattern',
            'thumbnailSrc':'static/images/thumbs/diagonal-pattern.jpg',
            'catLink1':'Design',
            'catLink2':'UI',
            'entryTitle':'You Can See Patterns Everywhere.',
            'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
                              ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
                              in anim.`
        },
        'dreamHost':{
            'linkUrl':'http://www.dreamhost.com/r.cgi?287326',
            'linkText':'http://www.dreamhost.com',
            'entryTitle':'Looking for affordable &amp; reliable web hosting? We recommend Dreamhost.'
        },
        'Gallery':{
			'type':'single-gallery',
			'slide1Src':'static/images/thumbs/gallery/work1.jpg',
			'slide2Src':'static/images/thumbs/gallery/work2.jpg',
			'slide3Src':'static/images/thumbs/gallery/work3.jpg',
			'catLink1':'Branding',
			'catLink2':'Wordpress',
			'entryTitle':'Workspace Design Trends and Ideas.',
			'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
					            ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
					            in anim.`
        },
        'USAF':{
            'type': 'single-standard',
            'srcAlt':'USAF rocket',
            'thumbnailSrc':'static/images/thumbs/usaf-rocket.jpg',
            'catLink1':'Branding',
            'catLink2':'Mockup',
            'entryTitle':'The 10 Golden Rules of Clean Simple Design.',
            'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
                              ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
                              in anim.`
        },
        'Shutterbug':{
            'type': 'single-standard',
            'srcAlt':'Shutterbug',
            'thumbnailSrc':'static/images/thumbs/shutterbug.jpg',
            'catLink1':'Photography',
            'catLink2':'html',
            'entryTitle':'Photography Skills Can Improve Your Graphic Design.',
            'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
                            ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
                            in anim.`
        },
        'quote':{
            'quote':'Good design is making something intelligible and memorable. Great design is making something memorable and meaningful.',
            'author':'Dieter Rams'
        },
        'concert':{
            'type': 'single-audio',
            'srcAlt':'concert',
            'thumbnailSrc':'static/images/thumbs/concert.jpg',
            'audioSrc':'static/media/AirReview-Landmarks-02-ChasingCorporate.mp3',
            'catLink1':'Design',
            'catLink2':'Music',
            'entryTitle':'This Is a Audio Format Post.',
            'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
                            ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
                            in anim.`
        },
        'ferris':{
            'type': 'single-standard',
            'srcAlt':'ferris wheel',
            'thumbnailSrc':'static/images/thumbs/ferris-wheel.jpg',
            'catLink1':'Design',
            'catLink2':'UI',
            'entryTitle':'This Is Another Standard Format Post.',
            'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
                            ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
                            in anim.`
        },
        'building':{
            'type': 'single-standard',
            'srcAlt':'building',
            'thumbnailSrc':'static/images/thumbs/diagonal-building.jpg',
            'catLink1':'Design',
            'catLink2':'Photography',
            'entryTitle':'Just a Standard Format Post.',
            'entryExcerpt':`Lorem ipsum Sed eiusmod esse aliqua sed incididunt aliqua incididunt mollit id et sit proident dolor nulla sed commodo est
                            ad minim elit reprehenderit nisi officia aute incididunt velit sint in aliqua cillum in consequat consequat in culpa
                            in anim.`
        }


}